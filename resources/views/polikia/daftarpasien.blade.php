    ...
    <div class="row">
        <div class="col-lg-12">
                ...
                @foreach ($patienttodays as $patienttoday)
                    ...
                    <td><a href="/polikia/pasien/{{$patienttoday->id}}/edit" class="text-decoration-none text-dark font-weight-bold">{{$patienttoday->member->nama}}</a></td>
                    <td>
                    @if ($patienttoday->diag_id)
                        {{$patienttoday->diag->diagnosa}}
                    @else
                        @if (count($patienttoday->labrecord))
                            <a href="/polikia/ceklab/{{$patienttoday->id}}/edit" class="">Form Cek-Lab</a>
                        @else
                            <a href="/polikia/ceklab/create/{{$patienttoday->id}}" class="">Form Cek-Lab</a>
                        @endif
                    @endif
                    </td>
                    </tr>
                ...
                @foreach ($patients as $patient)
                    ...
                    <a href="/polikia/pasien/{{$patient->id}}/edit" class="text-decoration-none text-dark font-weight-bold">{{$patient->member->nama}}</a>
                    @else
                    <a href="/polikia/pasien/create/{{$patient->id}}" class="text-decoration-none text-dark font-weight-bold">{{$patient->member->nama}}</a>
                    @endif
                    </td>
